...
namespace app\Table\Posts\UserTimeline;
...
class displayTimelineUserPostNotifyMyNetwork extends AppDisplay
{
    private $id;
    private $text;
    private $action;
    ...
    public function __construct($post, $user, $comments = [], $commentDisplayMode = false)
    {
        ...
        //traduce actions
        $langModel = new LangModel(App::getDatabase());
        $this->action = $langModel->getActionTraduce($this->action);
        ...
        return "
            <div class='post-header col-md-12'>
                <div class='post-intro col-md-7'>
                    ...
                    <div class='action'>
                        <p class='post-user-name'>". $this->user->nickname ."</p>
                        <p class='action-name'>". $this->action ."</p>
                    </div>
                    <div class='time'>
                    ". $this->date ."
                    </div>
    ...
    public function showPostBody()
    {
        ...
        //check current team : sinon affiche les previous teams : sinon la nationnalité
        if($this->qiUser->current_team)
        {
            $role = '<h3 class="role">'. $this->qiUser->role .' on '. $this->qiUser->game .' on '. $this->qiUser->platform .' - '. $this->qiUser->current_team .'</h3>';
        }
        else{
            if($this->qiUser->previous_team)
            {
                $role = '<h3 class="role">'. $this->langFile[$this->pageName]->title_previous_teams .' '. $this->qiUser->previous_team .'</h3>';
            }
            else{

                $role = '<h3 class="role">'. $this->langFile[$this->pageName]->title_nationnality .' '. $this->qiUser->nationnality .'</h3>';
            }
        }

        //check current company : sinon affiche le language
        if($this->qiUser->current_company)
        {
            $job = '<h3 class="job">'. $this->qiUser->jobtitle .' at '. $this->qiUser->current_company .'</h3>';
        }
        else{
            $job = '<h3 class="job">'. $this->langFile[$this->pageName]->title_languages .' : '. $this->qiUser->language .'</h3>';
        }
