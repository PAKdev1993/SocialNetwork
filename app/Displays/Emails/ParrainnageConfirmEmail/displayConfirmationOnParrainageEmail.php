<?php
namespace app\Displays\Emails\ParrainnageConfirmEmail;

use app\Table\AppEmailElems;
use app\Table\UserModel\UserModel;
use app\App;

class displayConfirmationOnParrainageEmail extends AppEmailElems
{
    private $userToSend;
    private $confirmlink;

    public function __construct($tokenConfirm, $useridToConfirm, $parrainageToken)
    {
        parent::__construct();
        $model =                new UserModel();
        $this->userToSend =     $model->getUserFromId($useridToConfirm);
        $this->confirmlink =    $this->baseLink . 'inc/Auth/confirm.php?id=' . $useridToConfirm . '&token=' . $tokenConfirm . '&tokenParrain=' . $parrainageToken;
    }
