...
class displayCommentNotifEmail extends AppEmailElems
{
    private $userTo;
    private $userFrom;
    ...
    public function __construct($useridFrom, $useridTo, $notifLink)
    {
        ...
        $commentNotif = 9; //code notif
        //#todo economie requette sur les deux fonctions suivantes
        $this->traducedMessage = $langModel->getMessageLangNotif($this->userTo->langWebsite, $commentNotif);
        $this->traducedBt = $langModel->getMessageLangBtNotifMail($this->userTo->langWebsite, $commentNotif);
        ...
        if($this->userFrom->background_profile)
        {
            ...
            $this->imgUser = str_replace($root, "http://worldesport.com/", $this->imgUser);
        }
        else{
            $this->imgUser = 'http://worldesport.com/public/img/default/defaultprofile.jpg';
        }
    }
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td width="10%" style="background-color:#F3F3F3;padding:5px;border-top: 2px solid rgba(243,177,75,1);border-left:2px solid rgba(243,177,75,1);border-top-left-radius:8px;" rowspan="3" align="center">
                                    <img style="width:50px; height:50px; margin:auto; border-radius:150px; border: 2px solid rgba(243,177,75,1);" src="'. $this->imgUser .'" alt="'. $this->userFrom->firstname .' \''. $this->userFrom->firstname .'\' '. $this->userFrom->lastname .' pic">
                                </td>
                                <td width="30%" style="font-weight:bold;background-color:#F3F3F3;font-family:\'Helvetica\',sans-serif;font-size:12px;border-top: 2px solid rgba(243,177,75,1);">
                                    '. $this->userFrom->firstname .' "'. $this->userFrom->firstname .'" '. $this->userFrom->lastname .'
                                </td>
                            ...
                            <tr>
                                <td style="border-left: 2px solid rgba(243,177,75,1);border-right: 2px solid rgba(243,177,75,1);font-size:20px;line-height:60px;text-align:right;padding-right:5%;" width="80%" height="10" colspan="6">
                                    ...
                                    <v:roundrect xmlns:v="urn:schemas-microsoft-com:vml" xmlns:w="urn:schemas-microsoft-com:office:word" href="'. $this->baseLink . $this->linkNotif .'" style="height:40px;v-text-anchor:middle;width:200px;" arcsize="25%" strokecolor="#ED7D31" fillcolor="#F9B14C">
                                        <w:anchorlock/>
                                        <center style="color:#ffffff;font-family:sans-serif;font-size:13px;font-weight:bold;">'. $this->traducedBt[0]->bt_view_post .'</center>
                                    </v:roundrect>
                                    <![endif]-->
                                    <a href="'. $this->baseLink . $this->linkNotif .'" style="background-color:#F9B14C;border:4px solid #ED7D31;border-radius:15px;color:#ffffff;display:inline-block;font-family:sans-serif;font-size:14px;font-weight:bold;line-height:40px;text-align:center;text-decoration:none;width:150px;-webkit-text-size-adjust:none;mso-hide:all;">'. $this->traducedBt[0]->bt_view_post .'</a>
                                </td>
                            </tr>                          
                            <tr>
